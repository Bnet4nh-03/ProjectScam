...
class QTIMTBAParsing(BaseApp):
    ...
    def add_event_error(self, path_file = "QTI_Jam_list.xlsx"):
    ...
    def create_last_marker(self, db_path):           
        ...
        cursor = conn.cursor()
        try:
            cursor.execute("ALTER TABLE HT9046_define ADD COLUMN last_marker TEXT")
        except sqlite3.OperationalError:
            pass
        
        cursor.execute("""
            UPDATE HT9046_define
               SET last_marker = ?
        """, ('',))
    ...
    def get_log_file(self, input_dir):
        ...
        subfolder_path = os.path.join(input_dir, year, month)
    ...
    def load_error_dict(self, file_path="error_dict.txt"):
    ...
    def get_error_stack(self, file, db_path, machine_name, mnbr, machine_type, last_marker):
        ...
        if last_marker:
            last_marker_date = datetime.strptime(last_marker, "%m/%d/%Y %H:%M:%S")
        else:
            last_marker_date = None

        #read logs by device type
        if machine_type == "HT-9046LS":
            try:
                with open(file, "r", encoding="utf-8", newline="") as f:
                    ...
                    for row in reader:
                        message  = row[7].strip() 
                        id_error = self.get_error_id(message, machine_type)
                        if id_error:
                            date_raw = row[0].strip() 
                            time_raw = row[1].split(".")[0].strip()

                            datetime_event_str = f"{date_raw} {time_raw}"
                            # print(datetime_event_str)
                            datetime_event = datetime.strptime(datetime_event_str, "%m/%d/%Y %H:%M:%S")
                            ...
                            if last_marker_date is None or datetime_event > last_marker_date:
                                ...
                                self.set_last_marker(
                                    db_path,
                                    machine_name,
                                    datetime_event.strftime("%m/%d/%Y %H:%M:%S")
                                )
                            else:
                                continue
            except Exception as e:
    ...
    def run(self):
        ...
        db_path = os.path.join(self.local_dir, "QTI_machine_define.db")
        copy_db_path = "QTI_machine_define.db"
        ...
        cursor = conn.cursor()
        cursor.execute("SELECT machine_name, input_dir, pattern, mnbr, machine_type, last_marker FROM HT9046_define")
