...
path_config = '/home/testit/SRC/SRC/src/KIOXIA/Fullcombine_ADV/path_config.ini'
...
def full_parsing(mypath: str):
    ...
    for fullpath in paths: #iterate each files in the folder
        ...
        if "_processed.asc" in file_temp:
            ...
            try:
                data = stored_procedure.get_lot_from_keyno(main_tracecode)
                if data:
        ...
        for trace_code in trace_code_list:
            ...
            data_list = ['get_lot_dcc', sub_tracecode]
            try:
                result = stored_procedure.get_dcc_data(data_list)
                lot_no = result.get('WPLOT#')
                dcc = result.get('WPDCC')
            except Exception:
                print(f"[{datetime.now(Vietnam_time).strftime('%Y-%m-%d %H:%M:%S')}] ERROR: can not find lot_no and dcc")
            # print(tempDf)
            ...
            try:
                if 'firstLine_S2_end_time' in locals() and firstLine_S2_end_time:
                    ...
                    startTime = half_first_line.split(',')[-8]
                    endTime = half_first_line.split(',')[-7]
                    # content S2 body
                    try:
            ...
            # call stored_procedure (advance)
            data_list = ['get_temperature', lot_no, dcc, programName]
            try:
            ...
            if get_opr_temp:
                try:
                    ...
                    print(f"[{datetime.now(Vietnam_time).strftime('%Y-%m-%d %H:%M:%S')}] ERROR: Can not find OPER_CODE")
                try:
                    TEST_STEP = str(get_opr_temp.get('WPOPRN'))
                except Exception:
                    print(f"[{datetime.now(Vietnam_time).strftime('%Y-%m-%d %H:%M:%S')}] ERROR: Can not find TEST_STEP")
                try:
                    TEMPERATURE = str(get_opr_temp.get('WPCOND')).strip().split('+')[0] + '.00'
                except Exception:
                    print(f"[{datetime.now(Vietnam_time).strftime('%Y-%m-%d %H:%M:%S')}] ERROR: Temperature not true")
            else:
                print(f"[{datetime.now(Vietnam_time).strftime('%Y-%m-%d %H:%M:%S')}] ERROR: Cannot get temperature or code station or test station. Please check \n{data_list}")
            ...
            # chuẩn biến cho generate_summary
            ...
            filename_dat = f"KIOXIA_{tester}_{PROGRAM_NAME}_{CUSTOM_LOT_NO}_{sub_tracecode}_{OPER_CODE}_{TEST_CODE}_{RETEST_CODE}_{current_time}.DAT"
            try:
