...
namespace Amkor.CADModules.ESIModule.Control
{
	...
	{
		// Token: 0x06000054 RID: 84 RVA: 0x00003CF0 File Offset: 0x00001EF0
		...
		// Token: 0x06000055 RID: 85 RVA: 0x00003FC4 File Offset: 0x000021C4
		...
		// Token: 0x06000056 RID: 86 RVA: 0x000043B8 File Offset: 0x000025B8
		...
		// Token: 0x06000057 RID: 87 RVA: 0x000046A8 File Offset: 0x000028A8
		...
		// Token: 0x06000058 RID: 88 RVA: 0x00004C00 File Offset: 0x00002E00
		...
		// Token: 0x06000059 RID: 89 RVA: 0x00004D58 File Offset: 0x00002F58
		...
		// Token: 0x0600005A RID: 90 RVA: 0x000051C4 File Offset: 0x000033C4
		public static bool Save(string filepath, Grid grid, bool visibleFlag, bool colorFlag)
		{
			int rowsCount = grid.RowsCount;
			int columnsCount = grid.ColumnsCount;
			object[,] array = new object[rowsCount, columnsCount];
			RangeRegion rangeRegion = new RangeRegion();
			object[,] array2 = new object[rowsCount, columnsCount];
			try
			{
				for (int i = 0; i < grid.RowsCount; i++)
				{
					for (int j = 0; j < grid.Columns.Count; j++)
					{
						...
						int columnSpan = grid[i, j].ColumnSpan;
						array[i, j] = grid[i, j].Value;
						array2[i, j] = grid[i, j].View.BackColor;
					}
				}
			}
			catch (Exception ex)
			...
			try
			{
				...
				for (int l = 0; l < rowsCount; l++)
				{
					...
					{
						if (!((Color)array2[l, m]).IsSystemColor)
						{
							worksheet.get_Range(worksheet.Cells.get__Default(l + 1, m + 1), worksheet.Cells.get__Default(l + 1, m + 1)).Interior.Color = array2[l, m];
						}
					}
				}
				workbook.SaveAs(filepath, Missing.Value, Missing.Value, Missing.Value, Missing.Value, Missing.Value, 1, Missing.Value, Missing.Value, Missing.Value, Missing.Value, Missing.Value);
